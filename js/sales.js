...
function orderUpdater() {
    getObjectStore('data', 'readwrite').get('soko-store-' + localStorage.getItem('soko-active-store') + '-orders').onsuccess = function (event) {
        ...
        for (var i = 0; i < reqs.length; ++i) {
            ...
            if (orderCrdRate == 0) {
                console.log(Math.ceil(getDistanceFromLatLonInKm(orderCrdLocationX, orderCrdLocationY, shopLocationX, shopLocationY)));
                $("#oderDeliveryRate_" + reqs[i].id).text(Math.ceil(getDistanceFromLatLonInKm(orderCrdLocationX, orderCrdLocationY, shopLocationX, shopLocationY)))
            } else {
                console.log(Math.ceil(getDistanceFromLatLonInKm(orderCrdLocationX, orderCrdLocationY, shopLocationX, shopLocationY) * orderCrdRate));
                $("#oderDeliveryRate_" + reqs[i].id).text(Math.ceil(getDistanceFromLatLonInKm(orderCrdLocationX, orderCrdLocationY, shopLocationX, shopLocationY) * orderCrdRate))
            }
        }
