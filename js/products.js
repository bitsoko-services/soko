    ...
    getObjectStore('data', 'readwrite').get('soko-store-' + localStorage.getItem('soko-active-store') + '-products').onsuccess = function (event) {
        ...
        for (var i = 0; i < reqs.length; ++i) {
            ...
            for (var pc = 0; pc < prodCat.length; ++pc) {
                $(".changeCategory-" + reqs[i].id + "").append('<option value="' + prodCat[pc].name + '">' + prodCat[pc].name + '</option>');
            }

            $(document).on('touchstart click', '.changeCategory-' + reqs[i].id + ' li', function (event) {
                var prodVal = $(this).find("input").context.innerText;
                ...
                doFetch({
                    ...
                    if (e.status == 'ok') {
                        Materialize.toast('Product category changed successfully', 3000);
                    } else {
