...
$(document).on("change", ".entSettings input", function (imgId) {
    ...
    } else if (entInput == "serviceImage") {
        console.log("do nothing")
        var currentInput = $(this);
        if (inputChanged.hasClass("serviceHeaderTwo")) {
            currentInput.parent().parent().parent().attr("getval", currentInput.val())
        } else if (inputChanged.hasClass("serviceHeaderOne")) {
            currentInput.parent().parent().parent().attr("getval", currentInput.val())
        } else if (inputChanged.hasClass("serviceBodyTwo")) {
            currentInput.parent().parent().parent().attr("getval", currentInput.val())
        } else if (inputChanged.hasClass("serviceBodyOne")) {
            currentInput.parent().parent().parent().attr("getval", currentInput.val())
        } else if (inputChanged.hasClass("serviceImageTwo")) {
            imgId = inputChanged
            base64Ent(imgId)
            currentInput.parent().parent().parent().attr("getval", srcData)
            alert(srcData)
        } else if (inputChanged.hasClass("serviceImageOne")) {
            imgId = inputChanged
            base64Ent(imgId)
            currentInput.parent().parent().parent().attr("getval", srcData)
            alert(srcData)
...
$(document).on("click", ".saveServiceOne", function () {
    ...
    for (var i = 0; i < allCards.length; ++i) {
        var icon = allCards[i].children[0].attributes[0].value
        var title = allCards[i].children[1].attributes[0].value
        var body = allCards[i].children[2].attributes[0].value
        sectionOneServArray = {
            icon,
            title,
            body
...
$(document).on("click", ".saveServiceTwo", function () {
    ...
    for (var i = 0; i < allCards.length; ++i) {
        ...
        servOneObj.push(sectionTwoServArray)
    }
