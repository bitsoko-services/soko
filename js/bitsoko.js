...
function profileLoaded(p){
    ...
    doFetch({action: 'merchantServiceLoader', id: p.bitsokoUserID }).then(function(e){
   ...
   loadPOS();
    ...
    doFetch({action: 'getMadr', id: p.bitsokoUserID }).then(function(e){
...
var test=new RegExp(cid).test(allCust[i].uid);
  if (test){
    ...
    swal({   title: "Message "+allCust[i].name,   text: 'send a short message to ',   type: "input", customClass: "salertHm", showCancelButton: true,   closeOnConfirm: false, confirmButtonText: "Send" ,   animation: "slide-from-top",   inputPlaceholder: "message", imageUrl: allCust[i].img.replace('50', '150') }, function(inputValue){   if (inputValue === false) return false;      if (inputValue === "") {     swal.showInputError("You need to write something!");     return false   } 
  
doFetch({action: 'sendMessage', to: 'user-'+allCust[i].uid, from: 'service-'+localStorage.getItem('bitsoko-merchant-id'), msg: inputValue }).then(function(e){  
    
    swal({   title: "Message Sent",   text: inputValue,   timer: 5000,   showConfirmButton: false }); });
        });
      break;
           }else{
               continue;
...
var test=new RegExp(cid).test(allCust[i].uid);
  if (test){
    ...
    swal({   title: "Send to "+allCust[i].name,   text: 'request a transfer from admin ',   type: "input", customClass: "salertHm", showCancelButton: true,   closeOnConfirm: false, confirmButtonText: "Request" ,   animation: "slide-from-top",   inputPlaceholder: "amount", imageUrl: allCust[i].img.replace('50', '150') }, function(inputValue){   if (inputValue === false ) return false;      if (inputValue === "" || isNaN(inputValue) ) {     swal.showInputError("You need to enter an amount!");     return false   } 
  
doFetch({action: 'sendTranReq', to: 'user-'+allCust[i].uid, from: 'service-'+localStorage.getItem('bitsoko-merchant-id'), amount: inputValue }).then(function(e){  
    
    swal({   title: "Request Sent",   text: inputValue,   timer: 5000,   showConfirmButton: false }); });
        });
      break;
           }else{
               continue;
...
function loadPOS(){
 ...
 var html = ' <li class="collection-item avatar closeSwitchStore" style="" svid="'+services[i].id+'"><img src="'+services[i].bannerPath+'" alt="" class="circle"><div class="row">'+
	     '<p class="collections-title">'+services[i].name+'</strong></p><p class="collections-content">...</p></div>'+
                         '</li>';
...
function Tobserver(changes){
  changes.forEach(function(change, i){
    ...
    bc.postMessage({cast: 'connect', user: 'serviceHandler' });
...
function refreshCustomers(){
   ...
   doFetch({action: 'getServiceCustomers', id : localStorage.getItem('soko-active-store')}).then(function (e){
...
function refreshProducts(){
 ...
 doFetch({action: 'getProducts', id : localStorage.getItem('soko-active-store')}).then(function (e){
...
function refreshPromotions(){	
 doFetch({action: 'getPromotions', id : id}).then(function (e){
...
function addStore(){
  ...
  updateMerch(e);
   ...
   id=e.id;
	refreshCustomers();
  doFetch({action: 'getServiceTrans', id : id}).then(function (e){
...
//          document.querySelector('.soko-tran-count').innerHTML = e.transactions.length;
 ...
 volFig = parseInt(e.transactions[i].amount)+volFig;
  ...
  doFetch({action: 'getServiceReqs', id : id}).then(function (e){
 ...
 doFetch({action: 'getBeacons', id : id}).then(function (e){
...
document.querySelector('.timeline').innerHTML = "";
    ...
    if(timeline.length==0){
         addTTL({time:0,type:'empty',title:'Nothing Found',body:'nothing to see here..'});
 ...
 console.log('trying to complete');
   ...
   console.log( $(this).attr('trnid') );
    ...
    swal({   title: "Complete Transaction",  customClass: "salertHm", text: ' enter transaction message to complete',   type: "input",   showCancelButton: true,   closeOnConfirm: false,  inputPlaceholder: "transaction message" }, function(inputValue){   if (inputValue === false) return false;      if (inputValue === "") {     swal.showInputError("You need to write something!");     return false   } 
  
doFetch({action: 'compTrn', trnid:trnid, msg: inputValue, to: 'user-'+currCust, from: 'service-'+localStorage.getItem('bitsoko-merchant-id')}).then(function(e){  
    
  doFetch({action: 'getServiceTrans', id : id}).then(function (e){
      
    swal({   title: "transaction completed",   text: inputValue,   timer: 5000,  showConfirmButton: false }); });
...
getObjectStore('data', 'readwrite').get('bitsoko-merchant-requests').onsuccess = function (event) {
  ...
  reqs = JSON.parse(reqs);
     for(var i = 0,cid=cid,timeline=timeline; i < reqs.length; ++i) {
      ...
      if(cid==reqs[i].reqto){
  timeline.push({time:moment(reqs[i].time).unix(),type:'message',title:'New Message',body:'',amount:reqs[i].amount}); 
...
getObjectStore('data', 'readwrite').get('bitsoko-merchant-transactions').onsuccess = function (event) {
 ...
 console.log(moment(reqs[i].posted).unix()); timeline.push({time:moment(reqs[i].posted).unix(),type:'sent',title:'Sent',body:'',amount:reqs[i].amount,status:reqs[i].status,transid:reqs[i].transid}); 
              
      }else{
          timeline.push({time:moment(reqs[i].posted).unix(),type:'rec',title:'Received',body:'',amount:reqs[i].amount,status:reqs[i].status,transid:reqs[i].transid}); 
	...
	console.log(reqs);
     ...
     // timeline.push({time:reqs[i].joined,uid:reqs[i].uid,type:'cust',title:reqs[i].name,body:reqs[i].servaccno,amount:reqs[i].img});
            var html = ' <li class="collection-item avatar">'+
                    '<img src="'+reqs[i].img+'" alt="" class="circle">'+
                    '<span class="title">'+reqs[i].name+'</span>'+
                    '<p>customer since<br> '+reqs[i].uid+'</p>'+
                  '</li>';
...
function createService(p){
    console.log(p);
    busDet={image:"assets/images/icon.png"};
    swal({   title: "Add Business Name",   text: 'add your business details below to start accepting payments.',   type: "input",   showCancelButton: false,   closeOnConfirm: false, confirmButtonText: "next", customClass: "salertHmInv",  inputPlaceholder: "name", imageUrl: busDet.image }, function(inputValue){   if (inputValue === false) return false;      if (inputValue === "") {     swal.showInputError("You need to write something!");     return false   }else if (inputValue.length > 20){
     swal.showInputError("thats too long!");     return false 
    } 
  busDet.name=inputValue;
    swal({   title: "Add Business Description",   text: 'add your business description below',   type: "input",   showCancelButton: false,   closeOnConfirm: false, confirmButtonText: "next", customClass: "salertHmInv",  inputPlaceholder: "description", imageUrl: busDet.image }, function(inputValue){   if (inputValue === false) return false;      if (inputValue === "") {     swal.showInputError("You need to write something!");     return false   }else if (inputValue.length > 140){
     swal.showInputError("thats too long!");     return false 
    } 
    
  busDet.desc=inputValue;

    swal({   title: "Add Business Logo",   text: 'add your business logo below', cancelButtonText: "add", confirmButtonText: "next",  showCancelButton: true,   closeOnCancel: false, closeOnConfirm: false, customClass: "salertHmInv", imageUrl: p.image.url.replace('50', '150') }, function(){ 
  
        //busDet.logo=inputValue

    swal({   title: busDet.name,   text: busDet.desc,   showCancelButton: false, confirmButtonText: "create", closeOnConfirm: false, customClass: "salertHmInv", imageUrl: p.image.url.replace('50', '150') }, function(){ 
...
//var html = ''+saleAmount+'</h5><small class="noteC-time text-muted">'+saleTime+'</small></div></div></a>';
	     var html = ' <li class="collection-item avatar"><i class="mdi-action-receipt green circle"></i><div class="row"><div class="col s5">'+
	     '<p class="collections-title"><strong>#3</strong> Tomatoes, sold</p><p class="collections-content">.......</p></div>'+
                         '<div class="col s2"><span class="task-cat green accent-2">P1</span></div><div class="col s5"><div class="progress"><div class="determinate green" style="width: 70%"></div>'+   
                         '</div><div class="select-wrapper initialized"><span class="caret">▼</span><select class="initialized"><option value="" disabled="" selected="">pending</option>'+
			'<option value="1">complete</option></select></div></div></div></li>'+
    $( ".soko-sales-list" ).append( $.parseHTML( html ) );
...
function noSalesUpdater(){
 ...
 var html = ' <li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-receipt grey circle"></i><div class="row">'+
	     '<p class="collections-title"><strong>No Transactions found</strong></p><p class="collections-content">you can start making sales using promotions </p></div>'+
                         '</li>';
...
function beaconsUpdater(){
 getObjectStore('data', 'readwrite').get('soko-store-'+localStorage.getItem('soko-active-store')+'-beacons').onsuccess = function (event) { 
     ...
     try{reqs = JSON.parse(reqs);}catch(err){reqs=[]};	    
        
    $( ".beacons-holda" ).html('');
	 if(reqs.length>0){
	 
	 	     var html = '<li class="collection-item avatar"><i class="mdi-action-settings-bluetooth cyan circle"></i>'+
                         '<span class="collection-header">All Beacons</span><p>'+reqs.length+' Found</p></li>';
	     $( ".beacons-holda" ).append( $.parseHTML( html ) );
	 }else{
	 
	 	     var html = '<li class="collection-item avatar"><i class="mdi-action-settings-bluetooth cyan circle"></i>'+
                         '<span class="collection-header">No Beacons Found</span><p>click here to add a beacon</p></li>';
	     $( ".beacons-holda" ).append( $.parseHTML( html ) );
	 }
     for(var i = 0; i < reqs.length; ++i) {
...
//var html = ''+saleAmount+'</h5><small class="noteC-time text-muted">'+saleTime+'</small></div></div></a>';
	     var html = '<li class="collection-item">'+
                         '<div class="row"><div class="col s5"><span class="task-cat pink accent-2">P1</span>'+
                          '<p class="collections-title"><strong>#'+reqs[i].name+'</strong> Connected</p><div class="select-wrapper initialized">'+
                    ...
                    '</select></div></div><div class="col s5"><div class="progress"><div class="determinate" style="width: 70%"></div>'+   
                     '</div></div></div></li>';
	     $( ".beacons-holda" ).append( $.parseHTML( html ) );
 ...
 getObjectStore('data', 'readwrite').get('soko-store-'+localStorage.getItem('soko-active-store')+'-products').onsuccess = function (event) { 
    ...
    $( ".products-collapsible" ).html('');
	 if(reqs.length==0){
	 	    ...
	 	    var html = ' <li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-shopping-basket grey circle"></i><div class="row">'+
	     '<p class="collections-title"><strong>No products found</strong></p><p class="collections-content">add a product to this store below</p></div>'+
                         '</li>';
...
//var html = ''+saleAmount+'</h5><small class="noteC-time text-muted">'+saleTime+'</small></div></div></a>';
	     var html = '<li prid="'+reqs[i].id+'" style="margin-bottom:10px; background: rgb(255, 255, 255);">'+
                    '<img id="prodImg-holda-'+reqs[i].id+'" src="'+reqs[i].imagePath+'" style=" width: 90px;display: inline-block;float: left;max-height: 86px;" alt="'+reqs[i].name+' image"><div class="collapsible-header" style="width: calc(100% - 90px);display:inline-block;">'+reqs[i].name+'<div class="divider"></div><span >'+
                    ...
                    '<form class="col s12" style="padding: 20px 30px;"><div class="row"><div class="input-field col s12">'+
                     '<input id="prodName-'+reqs[i].id+'" prnm="name" type="text" class="validate" prid="'+reqs[i].id+'" value="'+reqs[i].name+'"><label for="prodName-'+reqs[i].id+'" class="">Name</label></div></div>'+
                      '<div class="row"><div class="input-field col s12"><input prnm="description" placeholder="" value="'+reqs[i].description+'" id="prodDesc-'+reqs[i].id+'" type="text" class="validate" prid="'+reqs[i].id+'" min="0">'+
	      '<label for="description" class="">Description</label></div></div><div class="row">'+
		 '<div class="file-field input-field"><div class="btn"><span>image</span><input id="prodImg-'+reqs[i].id+'" prid="'+reqs[i].id+'" prnm="image" type="file">'+
                   '</div><div class="file-path-wrapper"><input class="file-path validate" type="text"></div></div>'+
		 '<div class="input-field col s6"><input prnm="price" placeholder="" value="'+reqs[i].price+'" id="prodPrice-'+reqs[i].id+'" type="number" class="validate" prid="'+reqs[i].id+'" min="0">'+
                     '<label for="prodPrice-'+reqs[i].id+'" class="active">Price</label></div><div class="input-field col s6">'+
                    '<div class="select-wrapper initialized"><span class="caret">▼</span><select id="prodMetric-'+reqs[i].id+'" prnm="metric" class="initialized" >'+
                      '<option value="" disabled="" selected="">measurement</option>'+
                      '<option value="1">per Kilogram</option>'+
                      '<option value="2">per Piece</option>'+
                    	'</select></div></div></div><div style="width: 100%;text-align: center;margin: 20px 0px 0px;color: rgba(0,0,0,0.4);">availability</div>'+
		 '<div class="row"><div class="input-field col s6">'+
                         '<input placeholder="" prnm="rstQuantity" id="prodRestNo-'+reqs[i].id+'" type="number" value="'+reqs[i].rstQuantity+'" class="validate" min="0" prid="'+reqs[i].id+'" max="1000">'+
                          '<label for="prodRestNo-'+reqs[i].id+'" class="active"> Quantity</label></div>'+
			'<div class="input-field col s6"><div class="select-wrapper initialized"><span class="caret">▼</span>'+
			'<select id="prodRestDur-'+reqs[i].id+'" prnm="rstDuration" class="initialized">'+
                      '<option value="" disabled="" selected="'+reqs[i].rstDuration+'">duration</option>'+
                      '<option value="day">per Day</option>'+
                      '<option value="week">per Week</option>'+
                      '<option value="month">per Month</option>'+
                    '</select></div></div></div>'+
	     '<div class="row" style="text-align: right;margin: 20px 0px;"> <a prid="'+reqs[i].id+'" class="removeProduct waves-effect waves-light btn">remove product</a> </div>'+
	     '</form></div></li>';
	     $( ".products-collapsible" ).append( $.parseHTML( html ) );
...
console.log(e);
	 if(e.length==0){
	 	     ...
	 	     var html = '<li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-redeem cyan circle"></i>'+
                         '<span class="collection-header">No Product Found</span></li>';
 ...
 getObjectStore('data', 'readwrite').get('soko-store-'+localStorage.getItem('soko-active-store')+'-promotions').onsuccess = function (event) { 
  ...
  reqs = JSON.parse(reqs);
	...
	if(reqs.length==0){
	 	     ...
	 	     var html = '<li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-redeem cyan circle"></i>'+
                         '<span class="collection-header">No Promotions Found</span></li>';
	     $( ".promotions-holda" ).append( $.parseHTML( html ) );
	 }else{
	 
    $( ".promotions-holda" ).html('');
 var html = ' <li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-redeem grey circle"></i><div class="row">'+
	     '<p class="collections-title"><strong>Add Promotion</strong></p><p class="collections-content">you can add '+(3-reqs.length)+' more promotions</p></div>'+
                         '</li>';
    $( ".promotions-holda" ).append( html );
     ...
     for(var i = 0; i < reqs.length; ++i) {
	  ...
	  //  var saleAmount=Math.ceil(parseFloat(reqs[i].amount)/100000000 *loCon.xrate*loCon.rate)+'/= '+loCon.symbol;
	     // var saleTime=moment(reqs[i].posted).fromNow();
//var html = ''+saleAmount+'</h5><small class="noteC-time text-muted">'+saleTime+'</small></div></div></a>';
	     var html = '<div id="promo-card" class="card"><div class="card-image waves-effect waves-block waves-light">'+
                      '<img class="activator" src="'+reqs[i].promoBanner+'" alt="user bg"></div><div class="card-content" style="padding: 0px 20px;">'+
                      '<img src="'+reqs[i].promoLogo+'" alt="" class="circle responsive-img activator card-profile-image">'+
                       '<a class="btn-floating activator btn-move-up waves-effect waves-light darken-2 right">'+
                        '<i class="mdi-editor-mode-edit" ></i></a><p>'+reqs[i].promoName+'</p><p>'+reqs[i].promoDesc+'</p>'+
                      '<p style="text-align: center;padding: 15px 20px;"><i style="float: left;" class="promo-state-icon mdi-notification-sync"> 0 shares</i>'+   
                      '<i class="promo-state-icon mdi-action-favorite"> 0 likes </i>'+
                      '<i style="float: right;" class="promo-state-icon mdi-action-receipt"> 0 sales </i></p>'+
		 
		 '<label>offer subscribers</label><div class="divider" style="margin: 10px;"></div><div class="promo-'+reqs[i].id+'-subscribers"></div>'+
		      '</div><div class="card-reveal">'+
		 '<span class="grey-text text-darken-4"><i class="card-title mdi-navigation-close right"></i></span>'+
                      '<div style="width: 100%;text-align: center;margin: 0px 0px 30px 0px;color: rgba(0,0,0,0.4);">promotion settings</div>'+
		 '<div class="row"><div class="input-field col s6"><input placeholder="" id="discount" type="number" class="validate" min="0">'+
                     '<label for="discount" class="active">% discount</label></div>'+
		 '<div class="input-field col s6"><input placeholder="" id="offers" type="number" class="validate" min="0">'+
                     '<label for="offers" class="active">minimum buyers</label></div></div>'+
		 '<div class="input-field col s12 m6"><select class="icons promo-add-ProdList" multiple></select><label>add an item to this promotion</label></div>'+

		 '<div class="row" style="text-align: right;margin: 20px 0px;"> <a class="waves-effect waves-light btn">remove promotion</a> </div>'+
'</div></div>';
...
var maxH=320;
  ...
  img.onload = function() {
    ...
    ctx.drawImage(img,0,0,iwScaled,ihScaled);
	  ...
	  doFetch({action: 'doProdUpdate', id : prid
		   ,prop: name, val: val}).then(function (e){
      if(e.status=='ok'){
...
function doSwitchStore(){
 ...
 var html = ' <li class="collection-item avatar" style="opacity: 0.6;"><i class="mdi-action-redeem grey circle"></i><div class="row">'+
	     '<p class="collections-title"><strong>changing store</strong></p><p class="collections-content">...</p></div>'+
                         '</li>';
...
function doNewStore(){
	...
	return;
	  ...
	  doFetch({action: 'doNewStore', ownerid : localStorage.getItem('bitsoko-owner-id')
		   , name: document.querySelector('#newStore-name').value
		  , desc: document.querySelector('#newStore-description').value
		  , loc: document.querySelector('#newStore-Location').value}).then(function (e){
      if(e.status=='ok'){
...
getObjectStore('data', 'readwrite').get('soko-store-customers-'+localStorage.getItem('soko-active-store')).onsuccess = function (event) {
  ...
  reqs = JSON.parse(reqs);
	console.log(reqs);
	 ...
	 doFetch({action: 'doCastPromo', id : $(t.target).attr('pid')
		   ,to: precp}).then(function (e){
      if(e.status=='ok'){
...
var maxH=320;
  ...
  img.onload = function() {
    ...
    ctx.drawImage(img,0,0,iwScaled,ihScaled);
	  ...
	  doFetch({action: 'doEditStore', id : localStorage.getItem('soko-active-store')
		   ,prop: name, val: val}).then(function (e){
      if(e.status=='ok'){
...
// value.addEventListener("change", updateStore);
	  ...
	  doFetch({action: 'doNewProduct', id : localStorage.getItem('soko-active-store') ,prod: newProdDat }).then(function (e){
      if(e.status=='ok'){
...
*/
    ...
    var shroot = document.querySelectorAll(".removeProduct");
	  ...
	  doFetch({action: 'doProdRemove', id : id }).then(function (e){
      if(e.status=='ok'){
...
var val=$(value).attr('prid');
	if (val=='undefined' || val==undefined){val=$(value).parent().parent().attr('prid')}
